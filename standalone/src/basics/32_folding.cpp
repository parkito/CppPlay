...
template <typename T> T sumAll(T last) { return last; }

template <typename T, typename... Args> auto sumAll(T first, Args... args) {
  return first + sumAll(args...);
}

// *************************************

template <typename... Args> void sumAll_2_fake_function(Args&&...) {}

template <typename T, typename... Args> auto sumAll_2(T first, Args... args) {
  T sum = first;
...
template <typename... Args> auto rightUnaryFold(Args... args) {
  // + - * / % ^ & | = < > << >> += -= *=
  // /= %= ^= &= |= <<= >>= == != <= >= && || , .* ->*
  return (args + ...);  // arg1 + (arg2 + (arg3 + (...)))
}

template <typename... Args> auto leftUnaryFold(Args... args) {
  return (... + args);  // ((arg1 + arg2) + arg3) + ...
}
...
// + - * / % ^ & | = < > << >> += -= *=
// /= %= ^= &= |= <<= >>= == != <= >= && || , .* ->*
template <typename... Args> void binaryFold(Args... args) {
  (std::cout << ... << args) << std::endl;
}

template <typename... Args> void binaryFold2(Args... args) {
  std::string result;
  (result += ... += std::to_string(args));
  std::cout << result << std::endl;
}

template <typename T, typename... Args> auto binaryFoldLeft(T sum, Args... args) {
  (sum += ... += args);
  return sum;
}

template <typename T, typename... Args> auto binaryFoldRight(T sum, Args... args) {
  (args += ... += sum);
  return sum;
}
...
int main() {
  ...
  std::cout << "sumAll = " << sumAll(std::to_string(1), std::to_string(2), std::to_string(3))
            << std::endl
            << std::endl;
  ...
  std::cout << "sumAll_2 = " << sumAll_2(std::to_string(1), std::to_string(2), std::to_string(3))
            << std::endl
            << std::endl;
  ...
  std::cout << "rightUnaryFold = "
            << rightUnaryFold(std::to_string(1), std::to_string(2), std::to_string(3)) << std::endl
            << std::endl;
  ...
  std::cout << "leftUnaryFold = "
            << leftUnaryFold(std::to_string(1), std::to_string(2), std::to_string(3)) << std::endl
            << std::endl;
  // std::cout << "leftUnaryFold = " << unaryFold("1", "2", "3", "4", "5") << std::endl;

  std::cout << "rightUnaryFold = \n"
            << rightUnaryFold(SumLog{1}, SumLog{2}, SumLog{3}, SumLog{4}, SumLog{5}) << std::endl
            << std::endl;
  // (1 + (2 + (3 + (4 + 5))))

  std::cout << "leftUnaryFold = \n"
            << leftUnaryFold(SumLog{1}, SumLog{2}, SumLog{3}, SumLog{4}, SumLog{5}) << std::endl
            << std::endl;
  // ((((1 + 2) + 3) + 4) +  5);
