...
template <typename T> struct Buffer {
  ...
  template <typename Iterator> Buffer(Iterator begin, Iterator end) : m_buffer{begin, end} {};

  template <typename P, typename U> Buffer(P *, U *){};
...
template <typename Iter> Buffer(Iter begin, Iter end)
    -> Buffer<typename std::iterator_traits<Iter>::value_type>;  // help compiler to deduce type in
                                                                 // very difficult situations

template <typename P, typename U> Buffer(P *, U *) -> Buffer<P>;

template <typename T> void zoo(T t) { std::cout << t << std::endl; }
