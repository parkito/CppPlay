...
template <typename T> T get_value(T t) {
...
template <typename T> T get_value2(T t) {
...
template <typename T> T get_value2(T* t) {
  ...
  if (t == nullptr) throw std::logic_error("nullptr detected!");
  return *t;
}

template <typename T> T get_value3(T t) {
...
int main() {
  ...
  // the compiler can't figure out what is the value will be provided
  //  get_value(value);
  //  get_value(&value);
